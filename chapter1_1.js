...
jsAnimationBtn.addEventListener('click', function moveBtnClickListener() {
    ...
    */
    ...
    function moveRightByFrame (timestamp) {
        if (!start) start = timestamp;
        var progress = timestamp - start;
        jsAnimationElement.style.left = Math.min(progress / 10, 100) + 'px';
        if (progress < 2000) {
            window.requestAnimationFrame(moveRightByFrame);
        }


    }

    // moveToRight(); uncomment to use moveToRight function that uses setTimeout

    window.requestAnimationFrame(moveRightByFrame); //uncomment to use moveRightByFrame function that uses requestAnimationFrame
